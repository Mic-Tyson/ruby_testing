...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        middle = game_guessing.make_guess
        expect(middle).to eq(4)
      end
    end
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_2) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        middle = game_2.make_guess
        expect(middle).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_2) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        middle = game_2.make_guess
        expect(middle).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_2) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        middle = game_2.make_guess
        expect(middle).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_2) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        middle = game_2.make_guess
        expect(middle).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      it 'is game over' do
        expect(game_guessing).to be_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      let(:number_guessing) { double('random_number', value: 8) }
      subject(:game_guessing) { described_class.new(0, 9, number_guessing, 9) }
      it 'is not game over' do
        expect(game_guessing).not_to be_over
      end
    end
  end
