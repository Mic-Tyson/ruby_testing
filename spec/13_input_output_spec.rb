...
# Long methods are like a run-on sentence that should have been divided into 2 or 3 different
# sentences so that everything could be clearly understood and in this case if a method does many
# different things it can be difficult to test.
...
describe NumberGame do
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        allow(game_loop).to receive(:player_input).and_return(invalid_char1, invalid_char2, valid_char)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      ...
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        loss_phrase = "That was hard. It took you 4 guesses!\n"
        expect { game.final_message }.to output(loss_phrase).to_stdout
      end
    end
  end
end
